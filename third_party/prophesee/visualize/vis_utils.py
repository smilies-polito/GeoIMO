...
def make_binary_histo(events, img=None, width=304, height=240):
    ...
    if img is None:
        ...
        img[...] = 127
    if events.size:
        in_bounds = (events['x'] < width) & (events['y'] < height) & (events['x'] >= 0) & (events['y'] >= 0)
        if not in_bounds.all():
            n_oob = int((~in_bounds).sum())
            print("Warning: skipping {} out-of-bounds events (x_max={}, y_max={}, w={}, h={})".format(
                n_oob, int(events['x'].max()), int(events['y'].max()), width, height))
